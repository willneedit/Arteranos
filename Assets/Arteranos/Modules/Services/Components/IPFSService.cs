...
namespace Arteranos.Services
{
    ...
    + "</html>\n";
        ...
        private readonly Dictionary<MultiHash, SDEntry> KnownServerDescriptions = new();
        ...
        private void DownloadServerDescription(MultiHash SenderPeerID, string sddPath)
        {
            ...
            async Task DownloadSDAsync(MultiHash SenderPeerID, string sddPath)
            {
                try
                {
                    ...
                    sd.LastSeen = now;
                    sd.DBUpdate();
                    KnownServerDescriptions.Remove(SenderPeerID);
                    KnownServerDescriptions.Add(SenderPeerID, new() 
                    { 
                        path = sddPath, 
                ...
                {
                    Debug.Log($"Timeout while downloading server description ({sddPath}) from {SenderPeerID}");
                }
            }
            ...
            if (KnownServerDescriptions.ContainsKey(SenderPeerID)
                && KnownServerDescriptions[SenderPeerID].path == sddPath)
            {
                SDEntry entry = KnownServerDescriptions[SenderPeerID];
                if (entry.lastSeen > now - TimeSpan.FromMinutes(30)) return;
